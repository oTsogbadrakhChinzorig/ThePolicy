module ThePolicy
  module Controller
    def login_required
      send ThePolicy.config.login_required_method
    end

    def policy_access_denied
      access_denied_method = ThePolicy.config.access_denied_method
      return send(access_denied_method) if access_denied_method && respond_to?(access_denied_method)

      the_policy_default_access_denied_response
    end
    ...
    def the_policy_default_access_denied_response
      access_denied_msg = t(:access_denied, scope: :the_policy)

      if request.xhr?
        render json: {
          errors: { the_policy: [ access_denied_msg ] },
